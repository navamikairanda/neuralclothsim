...
def get_config_parser():
    ...
    parser.add_argument('--boundary_condition_name', type=str, default='top_left_fixed', help='name of the spatio-temporal boundary condition; can be top_left_fixed, top_left_top_right_drape for rectangle, and two_rims_compression for cylinder, and top_rim_fixed, top_rim_torsion for cone')
    ...
    # training options
    parser.add_argument('--train_n_spatial_samples', type=int, default=400, help='N_omega, number of samples used for training; when reference geometry is an analytical surface, number of spatial grid samples along each curvilinear coordinate is square_root(N_omega)')
    parser.add_argument('--train_n_temporal_samples', type=int, default=10, help='N_t, number of temporal samples, at training')
    ...
    # debug options
    parser.add_argument('--debug', action='store_true', default=True, help='whether to run NeuralClothSim in debug mode; this will log the reference geometric quantities (e.g. metric and curvature tensor), the strains, and the simulated states to TensorBoard')
    parser.add_argument('--i_debug', type=int, default=200, help='frequency of Tensordboard logging of debug info')
    ...
    parser.add_argument('--test_n_temporal_samples', type=int, default=2, help='N_t, number of temporal samples, at training')
