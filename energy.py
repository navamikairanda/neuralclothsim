...
def compute_energy(deformations: torch.Tensor, ref_geometry: ReferenceGeometry, material: Material, external_load: torch.Tensor, temporal_coords: torch.Tensor, i: int, i_debug: int) -> torch.Tensor:   
    ...
    if isinstance(material, LinearMaterial):        
        ...
        hyperelastic_strain_energy_top = material.compute_internal_energy(strain, ref_geometry, material_directions, i, -0.5 * material.thickness)
        hyperelastic_strain_energy_mid = material.compute_internal_energy(strain, ref_geometry, material_directions, i, 0.)
        hyperelastic_strain_energy_bottom = material.compute_internal_energy(strain, ref_geometry, material_directions, i, 0.5 * material.thickness)                      
    ...
    if not i % i_debug and tb.writer:
        ...
        if ref_geometry.reference_midsurface.reference_geometry_name != 'mesh':
