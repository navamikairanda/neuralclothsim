...
class Energy:
    def __init__(self, ref_geometry: ReferenceGeometry, material: Material, external_load: torch.Tensor, i_debug: int):
    ...
    def __call__(self, deformations: torch.Tensor, temporal_coords: torch.Tensor, i: int) -> torch.Tensor:   
        ...
        if isinstance(self.material, LinearMaterial):        
            hyperelastic_strain_energy_mid = self.material.compute_internal_energy(strain, self.ref_geometry)
            ...
            hyperelastic_strain_energy_top = self.material.compute_internal_energy(strain, self.ref_geometry, material_directions, i, -0.5 * self.material.thickness)
            hyperelastic_strain_energy_mid = self.material.compute_internal_energy(strain, self.ref_geometry, material_directions, i, 0.)
            hyperelastic_strain_energy_bottom = self.material.compute_internal_energy(strain, self.ref_geometry, material_directions, i, 0.5 * self.material.thickness)                      
